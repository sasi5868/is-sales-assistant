...
exports.postAudio = async (req, res, next) => {
  ...
  console.log("saved audio file using multer", req.body.fileName);
  nlpResponse = {};
  languageId = req.body.languageId;
  inputFileName = req.body.fileName;
  console.log("languageId: ", languageId);

  asrInputFile = "/home/istl/demo/voice_channel_navigation/backend/data/audio/bbnlTest/" + inputFileName;

  if(languageId == 0) {
    asrResponseFileName = inputFileName.replace("wav", "txt");
    ...
    nlpResponseFileName = inputFileName.replace("wav", "json");
    ...
    console.log("LanguageId is incorrect");
    try {
      fs.unlinkSync(asrInputFile);
      console.log("audio file deleted successfully");
     } catch (e) {
      console.log("Unable to delete audio file");
     }
    res.status(200).json({
      ok: false,
      message: "LanguageId is incorrect",
      nlpResponse: {}
    });
    return;
  }
